...
父子组件之间使用 `props` 传递数据,传递方向是单向的，只能父组件数据变化传递给子组件。将父子组件解耦，避免子组件无意间修改父组件状态。
...
```html
<!-- 父组件中 -->
<chile-component @event='method'></chile-component>
```
$emit 为实例方法，用来触发事件监听。 event 代码自定义事件名称，参数 val 代表通过自定义事件传递的值，这里的 val 为可选参数.

> 子组件传值给父组件时，事件触发及接收原则是: 谁触发的监听谁接收

兄弟间组件通信的处理方式:通过共同的父组件进行中转

## slot 分发内容
slot 用于组件内容开发，分为两类
- 具名 slot: 设置特殊属性 name 来配置分发内容
- 匿名 slot: 没有名字的插槽，特点是可以放任何内容
# 计算属性和侦听器
- 计算属性: 把复杂的代码进行分离,定义在 computed 里,其中任何一个属性变化，计算属性都会重新执行，视图也会更新。
  - 与 methods 区别: 计算属性是基于依赖缓存的，一个计算属性所依赖的数据发生变化，它才会重新取值，所以属性只要不改变，计算属性就不更新。methods 时只要重新渲染，就会被调用，函数就会被执行
- 侦听器 观察和响应数据的变动,定义在 watch 里。大多数情况下使用计算属性，当需要在数据变化时执行异步或开销较大的操作，可以使用侦听器
# 插件的使用
- 百度地图插件
- vuex状态管理
- 二维码插件
